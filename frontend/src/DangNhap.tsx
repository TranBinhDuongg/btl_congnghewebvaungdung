...
import { apiLogin } from "./AuthHelper.ts";
...
export default function DangNhap() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!selectedRole) return setAlert({ msg: "Vui lòng chọn loại tài khoản", type: "error" });
    ...
    setAlert(null);
    try {
      const user = await apiLogin({ username, password, role: selectedRole });
      setAlert({ msg: `Chào mừng, ${user.tenHienThi}! Đang chuyển hướng…`, type: "success" });
      setTimeout(() => {
        const urls: Record<string, string> = {
          admin: "/admin", nongdan: "/nongdan", daily: "/daily", sieuthi: "/sieuthi",
        };
        window.location.href = urls[user.role] || "/";
      }, 1000);
    } catch (err: unknown) {
      setAlert({ msg: err instanceof Error ? err.message : "Đăng nhập thất bại", type: "error" });
    } finally {
      setLoading(false);
    }
  }
      ...
      <div style={{ background: G.white, borderRadius: 20, width: "100%", maxWidth: 420, boxShadow: "0 24px 64px rgba(0,0,0,0.28)", overflow: "hidden" }}>
        ...
        <form onSubmit={handleSubmit} style={{ padding: "28px 28px 24px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8 }}>
              {ROLES.map(r => (
                <button key={r.value} type="button" onClick={() => setSelectedRole(r.value)}
                  style={{ padding: "10px 4px", border: `2px solid ${selectedRole === r.value ? G.green : "#e0e0e0"}`, borderRadius: 10, background: selectedRole === r.value ? G.leaf : G.mist, cursor: "pointer", transition: "all .18s", textAlign: "center" }}>
                  <div style={{ fontSize: 18, marginBottom: 3 }}>{r.icon}</div>
                  <div style={{ fontSize: 10, fontWeight: 700, color: selectedRole === r.value ? G.green : "#888" }}>{r.label}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input style={fs("user")} value={username} onChange={e => setUsername(e.target.value)} onFocus={() => setFocused("user")} onBlur={() => setFocused(null)} placeholder="Nhập tên đăng nhập" autoComplete="username" />
          </div>
          ...
          <div style={{ marginBottom: 22 }}>
            ...
            <div style={{ position: "relative" }}>
              <input style={fs("pw")} type={showPw ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} onFocus={() => setFocused("pw")} onBlur={() => setFocused(null)} placeholder="Nhập mật khẩu" autoComplete="current-password" />
